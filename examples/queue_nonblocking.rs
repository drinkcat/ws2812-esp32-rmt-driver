...
fn main() -> ! {
    ...
    loop {
        ...
        match driver.write(&pixel_data) {
            Ok(()) => {
                let mut i = 0;
                // Since we do not block, this will fail a few times.
                while driver.write(&pixel_data).is_err() {
                    i += 1;
                }
        ...
        // However, write_blocking... blocks.
        log::info!("Calling write_blocking(), brightness={brightness}");
        match driver.write_blocking(&pixel_data) {
            Ok(()) => {
                let mut i = 0;
                // Since we block, this must immediately succeed.
                while driver.write_blocking(&pixel_data).is_err() {
                    i += 1;
                }
                if i == 0 {
                    log::info!(
